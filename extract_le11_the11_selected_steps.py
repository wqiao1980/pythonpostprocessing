...
"""Extract nodal LE11, THE11, and derived mechanical strain for a pipeline.

Run this file with the Abaqus Python interpreter. Examples:

    abaqus python extract_le11_the11_selected_steps.py --list-steps
    abaqus python extract_le11_the11_selected_steps.py --steps Step-2 Step-5
...
The default instance is PART-1-1. Results are ordered by cumulative distance
along its line-element mesh, starting from node set START when available.
...
def report_name(odb_path):
    ...
    return stem + "_LE11_THE11_MECH11.rpt"
...
def pipeline_graph(instance, coordinates):
    """Build path edges for two- and three-node beam/pipe elements."""
    graph = {}
    skipped_types = set()
    ...
    if not graph:
        raise ValueError(
            "No connected two- or three-node line elements were found in "
    ...
    if start_node is not None:
        if start_node not in graph:
            raise ValueError(
                ...
                "line element in instance '{1}'.".format(start_node, instance.name)
            )
        return start_node
...
def build_pipeline_paths(instance, start_node, start_node_set):
    ...
    if len(components) > 1:
        notes.append(
            "the instance contains {0} disconnected line-mesh paths; distance "
            "starts at zero for each path ID".format(len(components))
        )
    if skipped_types:
        notes.append(
            "non-line element types were excluded from path construction: {0}".format(
...
def extract_frame_rows(
    ...
    le_values, le_counts = collect_component_values(
        frame.fieldOutputs["LE"], "LE11", instance, average_section_points
    )
    the_values, the_counts = collect_component_values(
        frame.fieldOutputs["THE"], "THE11", instance, average_section_points
    )
...
def write_report(
    ...
    try:
        ...
        excel_plot_path = os.path.join(
            ...
            odb_stem + "_MAX_MECH11_along_path.xlsx",
        )
