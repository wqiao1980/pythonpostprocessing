...
def extract_s11_locations(
    ...
    for value in subset.values:
        ...
        if section_identity not in target_section_cache:
            section_angle = section_angle_from_label(section_label)
            target_section_cache[section_identity] = (
                section_angle is not None
                and target_angle(section_angle) is not None
            )
        if not target_section_cache[section_identity]:
            continue
        try:
...
def write_intermediate_report(
    ...
    with open(path, "w") as report:
        report.write(
            "Four-Angle Pipe S11 Step-Pair Intermediate Results\n"
        )
...
def scan_step_s11_envelope(
    ...
    for frame_index, frame in enumerate(frames):
        ...
        if intermediate_spool is not None:
            locations = sorted_locations(locations, route_distances)
...
def write_final_report(
    ...
    with open(path, "w") as report:
        ...
        report.write(
            "Section-angle filter during every frame scan: -90, 0, 90, "
